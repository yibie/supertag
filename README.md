...
| Save database, overwriting newer disk copy | `M-x supertag-save-store-force` | Intentionally writes this session's dirty Store over a newer on-disk revision |
...
**Multiple instances and revisions.** Every database save stamps a monotonic `:revision` and its writer. Any number of Emacs instances—daemon, GUI, or `emacsclient`—may open the same vault. A clean idle session silently follows a newer disk revision after `supertag-db-follow-interval` seconds (and when a View is refreshed). A save is refused only when this session has unsaved changes *and* the disk is newer; use `M-x supertag-reload-store` to discard those changes or `M-x supertag-save-store-force` to overwrite intentionally. Presence is informational only. Because cross-machine sync propagates with delay, revision following protects same-machine use, not concurrent edits on two machines.

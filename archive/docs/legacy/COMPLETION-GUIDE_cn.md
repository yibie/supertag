...
注意：`#` 是触发符，不是标签名称的一部分。下划线只是普通的标签字符，不表示层级。

### 嵌套标签

嵌套只认 `/`（中文输入法下的全角 `／` 同样可以）。输入 `#topics/emacs` 并选择 `[New]`，会创建缺少的 `topics` 和 `emacs`，并让 `emacs` 成为 `topics` 的子标签；正文里写入的是 `#emacs`。如果 `emacs` 已经属于别的父标签，`topics` 会作为第二个父标签加上去。

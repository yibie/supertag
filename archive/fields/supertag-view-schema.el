...
(defun supertag-schema-view-table-at-point ()
  ...
  (let ((query (supertag-view--get-tag-at-point)))
    ...
    (unless (require 'supertag-view-table nil t)
      (user-error "Table View is archived; explicitly add archive/table to load-path"))
          ...
          (progn
            (unless module
              (user-error "Managed Link Definition %s has no source module"
                          definition-id))
            (unless (require 'supertag-view-ontology nil t)
              (user-error "Ontology is archived; explicitly add archive/ontology to load-path"))

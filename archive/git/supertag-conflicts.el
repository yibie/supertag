...
raising `user-error')."
  (if (not (fboundp 'supertag-save-store))
    ...
    (if (and (fboundp 'supertag-dirty-p) (supertag-dirty-p))
        (message "Supertag: conflict resolution applied in memory, but the save was skipped just now (see the message above for why) -- the next auto-save will retry once resolved (or evaluate (supertag-save-store)).")

...
(defun supertag-view-kanban ()
  "Create an interactive Kanban view based on a tag's field."
  (interactive)
  (let* ((available-tags (supertag-view-kanban--get-all-tags))
    ...
    (if (not tag-id)
      ...
      (let* ((tag-data (supertag-tag-get tag-id))
             (display-name (plist-get tag-data :name))
             (fields (supertag-tag-get-all-fields tag-id))
             (field-names
              (let ((seen (make-hash-table :test 'equal))
                    names)
                (dolist (field fields (nreverse names))
                  (let* ((id (or (plist-get field :id)
                                 (plist-get field :name)))
                         (slug (and id (supertag-sanitize-field-id id))))
                    (when (and slug (not (gethash slug seen)))
                      (puthash slug t seen)
                      (push (plist-get field :name) names)))))))
        (if (not field-names)
            (message "Tag '%s' has no fields to group by." display-name)
          (let* ((field-name
                  (completing-read "Group columns by which field: "
                                   field-names nil t))
                 (config (supertag-view-kanban-create-config
                          tag-id field-name)))
            (when field-name
              (supertag-view-kanban-open config display-name)

...
(defun supertag-link-definition-find-by-ontology-key (module key)
  ...
  (seq-find
   (lambda (definition)
     (let ((authority
            (supertag-schema-authority-get :link (plist-get definition :id))))
       (and (eq module (or (plist-get definition :ontology-module)
                           (plist-get authority :module)))
            (eq key (or (plist-get definition :ontology-key)
                        (plist-get authority :key))))))
...
(defun supertag-link-definition-assert-editable (id)
  "Reject non-privileged mutation of ontology-managed definition ID."
  (when (and (supertag-link-definition-managed-p id)
...
(defun supertag-link-definition-update (id updater)
  ...
  (supertag-schema-authority-assert :link id :update)
  ...
  (let ((previous (supertag-link-definition-get id)))
    ...
    (let* ((candidate (funcall updater (copy-tree previous)))
      ...
      (when (and instances
        ...
        (cond
         ((eq supertag-schema-authority-current-actor :migration)
...
Refuse while concrete instances exist unless CASCADE is non-nil."
  (supertag-schema-authority-assert :link id :delete)
  (supertag-link-definition-assert-editable id)
  (let ((previous (supertag-link-definition-get id)))
    (unless previous
      (error "Link Definition %s does not exist" id))
...
(defun supertag-link-definition-reference (definition)
  ...
  (let* ((authority
          (supertag-schema-authority-get :link (plist-get definition :id)))
         (module (or (plist-get definition :ontology-module)
                     (plist-get authority :module)))
         (key (or (plist-get definition :ontology-key)
                  (plist-get authority :key))))
    (if (and module key)
              ...
              (cl-remove-if-not
               (lambda (definition)
                 (let ((authority
                   ...
                   (and (eq module
                            (or (plist-get definition :ontology-module)
                                (plist-get authority :module)))
                        (eq key
                            (or (plist-get definition :ontology-key)
                                (plist-get authority :key))))))
               definitions))))
     (text
      (let ((key (intern text)))
              ...
              (cl-remove-if-not
               (lambda (definition)
                 (let ((authority
                        (supertag-schema-authority-get
                         :link (plist-get definition :id))))
                   (or (eq key (or (plist-get definition :ontology-key)
                                   (plist-get authority :key)))

...
(declare-function supertag-load-store "supertag-core-persistence" (&optional file preserve-lock))
(declare-function supertag-discovery--load-history "supertag-discovery" ())
(declare-function supertag-automation--register-all-scheduled "supertag-automation" ())
...
active vault when `supertag-sync-auto-start` is non-nil."
  ...
  (unless (equal (plist-get vault :id) (supertag-vault--current-id))
    ...
    (when (fboundp 'supertag-automation--register-all-scheduled)

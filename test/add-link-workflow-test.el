   ...
   '((should (featurep 'supertag-service-org))
     ...
     (dolist (pair (append
                    ...
                    (if (or before (equal (getenv "SUPERTAG_VWC_STAGE") "before"))
                        '((supertag-view-helper-get-muted-color . "supertag-view-helper")
                          (supertag-view-helper-insert-section-title . "supertag-view-helper")
                          (supertag-view-helper-insert-simple-empty-state . "supertag-view-helper"))
                      '((supertag-view-helper-get-muted-color . "supertag-view-framework")
                        (supertag-view-helper-insert-section-title . "supertag-view-framework")
                        (supertag-view-helper-insert-simple-empty-state . "supertag-view-framework")))))
  ...
  (supertag-add-link-test--la-child
   ...
   '((let ((facts (prin1-to-string supertag--store)) (old (disk source)))
       ...
       (with-temp-buffer
         ...
         (should (string-match-p "No outgoing references" (buffer-string)))
         ...
         (let ((action (lookup-key (get-text-property (1- (point)) 'keymap) (kbd "RET"))))
           (should (commandp action)) (call-interactively action)
         ...
         ((or 'tag-merge 'tag-rename)
          ...
          (if (eq lc-case 'tag-merge)
            ...
            (let ((mapping '(("old" . "new"))))
              ...
              (setq mapping '(("new" . "a")))
              (let ((facts (prin1-to-string supertag--store)))
                (should-error (supertag-tag-path-rename--rewrite-relations mapping))
                (should (equal facts (prin1-to-string supertag--store))))))

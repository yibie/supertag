...
(ert-deftest supertag-automation-create-auc-property-template-persistence ()
  (supertag-automation-create-auc--child
   "properties"
   '(progn
      (require 'supertag-automation)
      (require 'document-fixture)
      (supertag-document-test-with-vault
        (let ((supertag--dirty nil)
              (supertag--rule-index (make-hash-table :test 'equal)) rules)
          (supertag-load-store)
          (should (eq 'complete (plist-get (supertag-reindex-org) :status)))
          (dolist (spec `((:property-change-update-property
                          (scope-tag . "scope") (source-property . "STATUS")
                          (target-property . "OTHER") (target-value . "changed"))
                         (:property-equals-move-node
                          (scope-tag . "scope") (property . "STATUS") (value . "go")
                          (target-file . ,(expand-file-name "archive.org" tmp)))))
            (let* ((tpl (cl-find (car spec) supertag-automation-templates
                                 :key (lambda (x) (plist-get x :id))))
                   (rule (supertag-automation-create
                          (funcall (plist-get tpl :build) (cdr spec)))))
              (push (copy-tree rule) rules)
              (should (eq :on-property-change (plist-get rule :trigger)))))
          (supertag--set-data-version supertag--store supertag-data-version)
          (supertag-mark-dirty)
          (supertag-save-store)
          (should (file-exists-p supertag-db-file))

...
| contextual | Separate ontology/link/reference suites (now `archive/ontology/tests/`) plus unlinked mentions |
...
These groups are not a claim that the new text-first model is implemented. Old schema/field/ontology tests still characterize the inherited runtime. The Discovery gate includes the matcher parity/performance file that remains outside the old default runner. The separate contextual suites now live under `archive/ontology/tests/` and are not covered by the old default suite.
...
| Archive | Table/kanban/schema-view, formula/aggregate/virtual-column/embed, schema/field-node-reference, reciprocal field migration, ontology suites (now under `archive/ontology/tests/`) | Preserve as archive compatibility evidence while consumers exist; remove from future default loading and release gate with the relevant cutover, not by silently weakening old tests now. |

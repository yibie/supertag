            ...
            `(progn
               ...
               (cl-labels ((disk (path) (with-temp-buffer (insert-file-contents path) (buffer-string)))
                 ...
                 (pcase ',operation
                   ...
                   ('bulk
                    (let ((saves nil) (projects nil) observer ids)
                      ...
                      (setq observer
                            (lambda (_loaded)
                              (when (featurep 'supertag-service-org)
                                ...
                                (advice-add 'supertag-service-org--project-current-node :before
                                            (lambda (id)
                      ...
                      (princ (format "D1-BULK-COUNTS saves=%S projects=%S\n" saves projects))
                      (should (equal (car ids) "stable"))
                      (should (= (length ids) 2))
                      (should-not (supertag-tag-resolve-occurrence "media"))
                      (dolist (pair ',(list (cons "document-node" file) (cons "second-node" plain)))
                        (should (= 1 (cl-count (car pair) projects :test #'equal)))
  ...
  (supertag-document-test-with-vault
    ...
    (require 'supertag-tag)) (d6-contract)
                    ...
                    (defun d6-sync-after-load (loaded)
                      (when (and (stringp loaded) (equal "supertag-services-sync" (file-name-base loaded)))
                        (advice-add 'supertag-node-sync-current-buffer :before #'d6-sync-observe)))
                ...
                (advice-remove 'supertag-node-sync-current-buffer #'d6-sync-observe)
                (mapc #'cancel-timer (append timer-list timer-idle-list)))))
        (with-temp-buffer
          (setenv "HOME" tmp) (setenv "CFFIXED_USER_HOME" tmp)
          (setenv "EMACSLOADPATH" (concat (mapconcat #'identity deps path-separator) path-separator))
    ...
    (dolist (mode '(light dark))
      (cl-letf (((symbol-function 'frame-parameter)
                 (lambda (frame parameter)
                   (if (eq parameter 'background-mode) mode (funcall real frame parameter)))))
        (let* ((muted (if (eq mode 'dark) "#AAAAAA" "#666666"))
               (emphasis (if (eq mode 'dark) "#99D6FF" "#003D82"))
               (border (if (eq mode 'dark) "#334155" "#E2E8F0"))
               (face `(:foreground ,emphasis :weight bold :box (:line-width 1 :color ,border))))
          (dolist (value '(nil ""))
            (should (equal-including-properties
                     (propertize "[No tags]" 'face `(:foreground ,muted :slant italic))
                     (supertag-view-helper-format-tag-value value))))
          (dolist (value '(" a, b " (" a " "b")))
            (let ((result (supertag-view-helper-format-tag-value value)))
              (should (equal-including-properties
                       (concat (propertize "#a" 'face face) " " (propertize "#b" 'face face)) result))
                      ...
                      (dolist (feature '(supertag-view-helper supertag-view-api supertag-query supertag-services-query supertag-services-note-query
                        ...
                        (should-not (cl-find-if
                                     (lambda (row) (and (stringp (car row))
                                                       (equal (file-name-base (car row)) (symbol-name feature)))) load-history)))
                      (should-not (file-exists-p supertag-db-file))
                      (dolist (symbol '(supertag-view-helper-get-muted-color supertag-view-helper-get-emphasis-color
                                         supertag-view-helper-get-border-color))
                        (should (autoloadp (symbol-function symbol)))))
                    (setq supertag--store (with-temp-buffer (insert-file-contents ,snapshot) (read (current-buffer))))
                    (let ((facts (prin1-to-string supertag--store))
                      ...
                      (if (eq ',operation 'query)
                        ...
                        (let ((real (symbol-function 'frame-parameter)))
                          (cl-letf (((symbol-function 'frame-parameter)
                                     (lambda (frame parameter)
                                       (if (eq parameter 'background-mode) 'dark (funcall real frame parameter)))))
                            (should (equal-including-properties
                                     (if (eq ',operation 'empty)
                                         (propertize "[No tags]" 'face '(:foreground "#AAAAAA" :slant italic))
                                       (propertize "#canonical" 'face
                                                   '(:foreground "#99D6FF" :weight bold :box (:line-width 1 :color "#334155"))))
                                     (supertag-view-helper-format-tag-value ,(unless (eq operation 'empty) "canonical")))))

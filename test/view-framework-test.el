  ...
  (supertag-view-framework-vwb--child
   "helper" '(progn
     ...
     (let ((text (supertag-view-helper-format-tag-value nil)))
       ...
       (should (plist-get (get-text-property 0 'face text) :foreground)))
  ...
  (supertag-view-framework-vwb--child
   "link" '(progn
     ...
     (with-temp-buffer
       ...
       (should (string-match-p "No outgoing" (buffer-string))))
...
(defconst supertag-view-framework-vwc--drawing-symbols
  '((supertag-view-helper-insert-action-button defun) (supertag-view-helper--section-separator defvar) (supertag-view-helper--subsection-separator defvar) (supertag-view-helper-insert-section-header defun) (supertag-view-helper-insert-subsection-header defun) (supertag-view-helper-get-theme-adaptive-color defun) (supertag-view-helper-get-accent-color defun) (supertag-view-helper-get-emphasis-color defun) (supertag-view-helper-get-muted-color defun) (supertag-view-helper-get-success-color defun) (supertag-view-helper-get-warning-color defun) (supertag-view-helper-get-error-color defun) (supertag-view-helper-get-background-color defun) (supertag-view-helper-get-border-color defun) (supertag-view-helper-format-value defun) (supertag-view-helper-render-org-links defun) (supertag-view-helper-format-boolean-value defun) (supertag-view-helper-format-number-value defun) (supertag-view-helper-format-date-value defun) (supertag-view-helper-format-url-value defun) (supertag-view-helper-insert-simple-header defun) (supertag-view-helper-insert-section-title defun) (supertag-view-helper-insert-simple-footer defun) (supertag-view-helper-insert-simple-empty-state defun) (supertag-view-helper-insert-card-start defun) (supertag-view-helper-insert-card-content defun) (supertag-view-helper-insert-card-end defun) (supertag-view-helper-insert-separator-line defun) (supertag-view-helper-highlight-current-line defun) (supertag-view-helper-unhighlight-all-lines defun) (supertag-view-helper-enable-line-highlighting defun) (supertag-view-helper-insert-status-badge defun) (supertag-view-helper-insert-stats-summary defun) (supertag-view-helper-insert-help-text defun) (supertag-view-helper-insert-empty-state defun) (supertag-view-helper-insert-node-info defun) (supertag-view-helper-insert-footer-start defun) (supertag-view-helper-insert-footer-with-content defun) (supertag-view-helper-display-buffer-right defun)))
  ...
  (supertag-view-framework-vwc--child
   ...
   `(progn
      ...
      (let ((fn (symbol-function 'supertag-view-helper-insert-action-button))
            (separator supertag-view-helper--section-separator))
        (require (if before 'supertag-view-helper 'supertag-view-framework))
        (should (eq fn (symbol-function 'supertag-view-helper-insert-action-button)))
        (should (eq separator supertag-view-helper--section-separator)))
     ...
     (let ((data (list :node "N")) seen)
       (with-temp-buffer
         ...
         (let ((b (button-at (point-min))))
           ...
           (should (keymapp (button-get b 'keymap)))
           (button-activate b)
           (princ (format "VWC-BUTTON-ACTION %S\n" seen))
           (should (equal seen '((:node "N")))))
         (insert " ")
         (let ((begin (point)))
           (supertag-view-helper-insert-action-button
            "Custom" (lambda (b) (push (button-get b 'custom-vwc) seen)) data "Custom help" 'custom-vwc)
           (button-activate (button-at begin))
           (should (eq (car seen) data)))
         (should (equal (buffer-string) "Execute Custom"))))
     (let ((original (symbol-function 'frame-parameter)))
       (dolist (mode '(light dark))
         (cl-letf (((symbol-function 'frame-parameter)
                    (lambda (frame parameter)
                      (if (eq parameter 'background-mode) mode (funcall original frame parameter)))))
           (should (equal (supertag-view-helper-get-theme-adaptive-color "L" "D") (if (eq mode 'dark) "D" "L")))
           (let ((text (supertag-view-helper-format-value nil)))
             (should (equal (substring-no-properties text) "[Empty]"))
             (should (equal (plist-get (get-text-property 0 'face text) :foreground)
                            (if (eq mode 'dark) "#AAAAAA" "#666666"))))
           (should (equal (substring-no-properties (supertag-view-helper-format-value '("A" "B"))) "A / B"))
           (dolist (pair '((supertag-view-helper-format-boolean-value "yes" "✓ True")
                           (supertag-view-helper-format-number-value 12 "12")
                           (supertag-view-helper-format-date-value "2026-09-08" "📅 2026-09-08")
                           (supertag-view-helper-format-url-value "https://invalid.example" "🔗 https://invalid.example")))
             (let ((text (funcall (car pair) (cadr pair))))
               (should (equal (substring-no-properties text) (caddr pair)))
               (should (get-text-property 0 'face text))))))))))
...
(ert-deftest supertag-view-framework-vwc-components-overlays-and-window ()
  (supertag-view-framework-vwc--child
   "components" '(progn
     (require (if before 'supertag-view-helper 'supertag-view-framework))
     (with-temp-buffer
       (supertag-view-helper-insert-section-header "Header" "H")
       (supertag-view-helper-insert-subsection-header "Subheader")
       (supertag-view-helper-insert-simple-header "Simple" "Stats")
       (supertag-view-helper-insert-section-title "Section" "S")
       (supertag-view-helper-insert-card-start "Card")
       (supertag-view-helper-insert-card-content "First\nSecond")
       (supertag-view-helper-insert-card-end)
       (supertag-view-helper-insert-separator-line :dotted)
       (supertag-view-helper-insert-status-badge 'active "Alive")
       (supertag-view-helper-insert-stats-summary '(:total 4 :active 2 :modified 1))
       (supertag-view-helper-insert-node-info "Node" "/tmp/display-only.org")
       (supertag-view-helper-insert-simple-empty-state "Empty one")
       (supertag-view-helper-insert-empty-state "Empty two")
       (supertag-view-helper-insert-simple-footer "Help one")
       (supertag-view-helper-insert-footer-with-content "Help two")
       (dolist (word '("Header" "Subheader" "Simple" "Section" "Card" "First" "Second"
                        "Alive" "Total: 4" "Node" "display-only.org" "Empty one" "Empty two" "Help one" "Help two"))
         (should (string-match-p (regexp-quote word) (buffer-string))))
       (should (string-match-p (regexp-quote supertag-view-helper--section-separator) (buffer-string)))
       (should (string-match-p (regexp-quote supertag-view-helper--subsection-separator) (buffer-string))))
     (with-temp-buffer
       (insert "one\ntwo\n") (goto-char (point-min))
       (let ((post-command-hook nil))
         (supertag-view-helper-highlight-current-line)
         (let ((ov (car (overlays-in (point-min) (point-max)))))
           (should (= (overlay-start ov) 1)) (should (= (overlay-end ov) 5))
           (should (eq (overlay-get ov 'category) 'current-line))
           (should (= (overlay-get ov 'priority) 100)) (should (overlay-get ov 'face)))
         (supertag-view-helper-highlight-current-line)
     ...
     (save-window-excursion
       (let ((buffer (generate-new-buffer " *VWC window*")))
         (unwind-protect
             (progn
               (with-current-buffer buffer (insert "first\nlast\n") (goto-char (point-max)))
               (supertag-view-helper-display-buffer-right buffer)
               (should (eq (window-buffer (selected-window)) buffer))
               (should (= (point) (point-min)))
               (princ (format "VWC-WINDOW point=%S start=%S\n" (point) (window-start))))
           (kill-buffer buffer)))))))
  ...
  (supertag-view-framework-vwc--child
   "reload" '(progn
     ...
     (let* ((registry supertag--view-registry) (configs supertag--view-configs)
            ...
            (separator (copy-sequence supertag-view-helper--section-separator))
       ...
       (should (eq subscribers supertag--subscribers))
       (should (equal separator supertag-view-helper--section-separator))
     ...
     `(progn
        ...
        ,(if (eq entry 'tag)
             '(let ((text (supertag-view-helper-format-tag-value '("red" "blue"))))
                ...
                (should (eq (plist-get (get-text-property 0 'face text) :weight) 'bold))
                (should (plist-get (plist-get (get-text-property 0 'face text) :box) :color)))
           '(let ((target (expand-file-name "target.org" vwc-tmp)))
              ...
              (let ((facts (prin1-to-string supertag--store))
                ...
                (with-temp-buffer
                  ...
                  (let ((action (lookup-key (get-text-property (1- (point)) 'keymap) (kbd "RET"))))
                    (call-interactively action)
       ...
       (let ((target (expand-file-name "target.org" tmp)))
         ...
         (let ((facts (prin1-to-string supertag--store)) (disk (supertag-document-test-disk file))
           ...
           (with-temp-buffer
             ...
             (should (string-match-p "Similar notes (candidates)" (buffer-string)))
           ...
           (with-temp-buffer
             ...
             (should (string-match-p "Unlinked Mentions (1)" (buffer-string)))

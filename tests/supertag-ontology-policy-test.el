...
(ert-deftest supertag-v11-node-view-lists-policy-aware-action-without-preconditions ()
  (supertag-v10-test-with-store
    (supertag-v10-test-install-status-action)
    (let ((called 0))
      (cl-letf (((symbol-function 'supertag-ontology-function-call)
        ...
        (with-temp-buffer
          (supertag-view-node--insert-ontology-capabilities-section
           "project-1")
          (should (string-match-p "Complete \\[Run\\]" (buffer-string))))
        (should (= called 0))))))
